...
void CDRConn::parse_buffer(uint8_t start, uint8_t *buf, const size_t bufsize, size_t bytes_received)
{
	...
	uint16_t crc = crc16(buf + start + sizeof(struct Header), len);
	uint16_t read_crc = ((uint16_t)header->crc_h << 8) | header->crc_l;
	memmove(buffer, buf + start + sizeof(struct Header), len);
	memmove(buf + start, buf + start + sizeof(struct Header) + len, sizeof(buf) - start - sizeof(struct Header) - len);
	if (crc == read_crc)
		if (message_received_cb)
			message_received_cb(topic_id, len, buffer);
}


uint16_t CDRConn::crc16_byte(uint16_t crc, const uint8_t data)
{
    return (crc >> 8) ^ crc16_table[(crc ^ data) & 0xff];
}

uint16_t CDRConn::crc16(uint8_t const *buffer, size_t len)
{
    uint16_t crc = 0;
    while (len--)
